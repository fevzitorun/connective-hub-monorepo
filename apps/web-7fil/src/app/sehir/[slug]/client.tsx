...
import { BrandSilhouette, IconSearch, IconSparkle, IconMapPin, IconChart, IconBed, IconArea } from "@/components/icons";
import { Photo } from "@/components/photos";
import type { Listing } from "@/lib/data";

const ListingCard = ({ l, onOpen }: { l: Listing; onOpen?: (l: Listing) => void }) => (
  <article
    className="rounded-2xl overflow-hidden border border-stone-200 bg-white shadow-sm hover:shadow-md transition-shadow cursor-pointer"
    onClick={() => onOpen && onOpen(l)}
  >
    <div className="aspect-[4/3] overflow-hidden"><Photo scene={l.scene} palette={l.palette} /></div>
    <div className="p-4">
      <div className="text-lg font-bold text-amber-600">{fmtTRY(l.price)}{l.kind === "kiralik" ? <span className="text-xs font-normal"> /ay</span> : null}</div>
      <div className="font-semibold text-sm mt-1 line-clamp-2">{l.title}</div>
      <div className="flex items-center gap-1 text-xs text-stone-500 mt-1"><IconMapPin size={12} /> {l.city}</div>
      <div className="flex gap-3 text-xs text-stone-400 mt-2">
        <span className="flex items-center gap-1"><IconBed size={12}/> {l.rooms}</span>
        <span className="flex items-center gap-1"><IconArea size={12}/> {l.area} m²</span>
      </div>
    </div>
  </article>
);
