...
const SceneApartment = ({ p }: { p: string[] }) => (
  <g>
...
const SceneHistoric = ({ p }: { p: string[] }) => (
  <g>
    <rect width="400" height="300" fill={p[0]}/>
...
const SCENES: Record<string, React.ComponentType<{ p: string[] }>> = {
...
export const Photo = ({ scene = 'apartment', palette = 'warm', caption, children }: PhotoProps) => {
  const Scene = SCENES[scene] ?? SCENES['apartment'] ?? SceneApartment;
