...
/* Curated conversation that demonstrates the "wise legal grandfather" voice
   and the natural-language search → listing card flow. */
...
const AtlasScreen = ({ onOpenListing }) => {
  const [input, setInput] = useState('');
  const [visible, setVisible] = useState(SCRIPT.length); // all messages preloaded for prototype
        ...
        <header className="atlas-main-head">
          ...
          <div style={{marginLeft:'auto', display:'flex', gap:8}}>
            <span className="pill pill-line" style={{height:24, fontSize:11}}>
              <window.IconSparkle size={12}/> claude-sonnet-4
            </span>
            <button className="btn btn-ghost btn-sm">
              <window.IconKebab size={16}/>
            </button>
          </div>
        </header>
          ...
          {SCRIPT.slice(0, visible).map((m, i) => (
            <div key={i} className="msg" data-from={m.from}>
              <div className="msg-avatar">
                {m.from === 'ai' ? <window.IconCompass/> : 'ZA'}
              </div>
              <div>
                  ...
                  {m.listings && (
                    <div className="msg-listings">
                      {m.listings.map(id => {
                        ...
                        if (!l) return null;
                        return (
                          <div key={id} className="msg-listing" onClick={() => onOpenListing(l)}>
                            ...
                            <div className="msg-listing-price">{window.fmtTRY(l.price)}</div>
                          </div>
                        );
                      })}
                    </div>
