...
import {
  IconArrowLeft, IconSparkle, IconMapPin, IconStar, IconWhatsapp, IconChat,
  BrandSilhouette,
} from '@/components/icons';
import { Pill, ListingCard, RitualVertical } from '@/components/ui';
import { Photo } from '@/components/photos';
import type { Listing } from '@/lib/data';

const FeatRow = ({ label, value }: { label: string; value: string }) => (
...
export default function DetailScreen() {
  ...
  const handleOpenListing = (r: Listing) => {
    router.push(`/ilanlar/${r.id}`);
  };

  return (
    <div className="detail-wrap">
      <button className="detail-back" onClick={() => router.push('/ilanlar')}>
        <IconArrowLeft/> Tüm ilanlara dön
      </button>

      {/* Gallery */}
      <div className="detail-gallery">
        <Photo scene={l.scene} palette={l.palette} caption={`${l.district} · ${l.city.split(' · ')[1]||''}`}/>
        <Photo scene="apartment" palette="morn" caption="Salon"/>
        <Photo scene="historic" palette="warm" caption="Mutfak"/>
        <Photo scene="coast" palette={l.palette} caption="Manzara"/>
        <Photo scene="villa" palette="cool" caption="Banyo"/>
      </div>
      ...
      <div className="detail-cols">
        ...
        <div className="detail-main">
          <div style={{display:'flex',alignItems:'center',gap:8,marginBottom:8}}>
            ...
            <Pill kind="line">{l.rooms}</Pill>
            {isRent && <Pill kind="warn">7fil Güvence</Pill>}
            <span style={{fontSize:12, color:'var(--muted)', marginLeft:'auto'}}>İlan no · 7F-{l.id.toUpperCase()}-2026</span>
          </div>

          <h1>{l.title}</h1>
          <div className="detail-loc"><IconMapPin/> {l.district}, {l.city}</div>
          ...
          <div className="detail-features">
            ...
            {isRent && l.deposit && <FeatRow label="Depozito" value={l.deposit + ' aylık kira'}/>}
            <FeatRow label="Özellikler" value={l.features.slice(0,3).join(' · ')}/>
            <FeatRow label="Atlas AI değerleme" value={l.aiValue ? fmtTRY(l.aiValue) : 'Hazırlanıyor'}/>
          ...
          <div className="atlas-report">
            <div className="atlas-report-watermark" aria-hidden="true">
              <BrandSilhouette color="rgba(201,168,76,0.6)"/>
            </div>
            <div className="atlas-report-head">
              <span className="atlas-report-chip"><IconSparkle/> Atlas AI</span>
          ...
          <div style={{marginTop:64}}>
            ...
            <div className="cards-grid">
              {related.map(r => <ListingCard key={r.id} l={r} onOpen={handleOpenListing}/>)}
            </div>
          </div>
        </div>
          ...
          <div className="detail-side-card">
            ...
            <div className="detail-side-tax">
              {isRent
                ...
                : `Tapu masrafı yaklaşık ${fmtTRY(Math.round(l.price * 0.04))}`}
            </div>
            ...
            <div className="detail-agent">
              ...
              <div className="col" style={{gap:2}}>
                ...
                <span className="detail-agent-rating"><IconStar/> {l.agent.rating.toFixed(1)} · 142 referans</span>
              </div>
            </div>
            ...
            <div className="detail-actions">
              ...
              <button className="btn btn-whatsapp">
                <IconWhatsapp size={18}/> WhatsApp ile Sor
              </button>
              <button className="btn btn-outline">
                <IconChat size={16}/> Atlas AI ile Tartış
              </button>
            </div>
          </div>
